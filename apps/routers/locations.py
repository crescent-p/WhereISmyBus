...
@router.get('/', status_code=status.HTTP_200_OK, response_model=schemas.FetchedLocation)
async def get_bus_locations(secret: str, key: str, redis: Session = Depends(get_redis)):
    value = redis.get(key)
    if value is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Location not found")
    return {"message": "all locations revealed", "location": value}

@router.post('/', status_code=status.HTTP_200_OK, response_model=schemas.UpdatedLocation)
async def set_bus_locations(secret: str, key: str, value: str, redis: Session = Depends(get_redis)):
    redis.set(key, value)
